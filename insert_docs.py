                    ...
                    for link in result.links.get("internal", []):
                        ...
                        for link in result.links.get("internal", []):
                            ...
                            if next_url not in visited:
                                # Filtro de permissão/negação
                                if allow_patterns and not any(re.match(p, next_url) for p in allow_patterns):
                                    continue
                                if deny_patterns and any(re.match(p, next_url) for p in deny_patterns):
                                    continue
...
def main():
    ...
    #     crawl_results = asyncio.run(crawl_recursive_internal_links([url], max_depth=args.max_depth, max_concurrent=args.max_concurrent))
        ...
        # Define the base path you want to stay inside
        base_pattern = r"^https://www\.gov\.br/saude/pt-br/vacinacao"

        crawl_results = asyncio.run(crawl_recursive_internal_links(
            [url],
            max_depth=args.max_depth,
            max_concurrent=args.max_concurrent,
            allow_patterns=[base_pattern],
            deny_patterns=[r"^(?!https://www\.gov\.br/saude/pt-br/vacinacao).*"]
        ))
